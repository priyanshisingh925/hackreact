...
// Allow all origins — required for Render cross-service deployment
app.use(cors({ origin: '*' }));
...
app.get('/api/health', (_req, res) => res.json({ status: 'ok', ts: Date.now(), accounts: store.accounts.length }));
...
async function bootstrap() {
  ...
  // ─── Random Forest (pure JS — always works) ───
  ...
  // ─── Neural Network (TF.js — graceful fallback if unavailable) ───
  let nnReady = false;
  try {
    ...
    console.warn('[ML]     Neural Network unavailable — hybrid scoring will use RF×0.8 + heuristic×0.2');
    console.warn('[ML]    ', e.message);
  }
  store.modelMetrics.lastTrainedAt = new Date().toISOString();
  store.modelMetrics.nnReady = nnReady;

  // ─── Pre-score all accounts ───────────────────────────────────────
  ...
  console.log('[ML]     Pre-scoring accounts...');
  let scored = 0;
  ...
  // ─── Start listening — bind 0.0.0.0 required for Render ──────────
